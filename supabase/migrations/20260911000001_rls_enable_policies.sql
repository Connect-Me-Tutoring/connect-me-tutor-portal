...
-- Legacy policies pre-dating this migration. Without dropping these, the
-- USING(true) grants below stay in force alongside the restrictive ones
-- (RLS policies are OR'd together), so nothing here would actually restrict
...
-- other service-written logs (see zoom_participant_events below).
